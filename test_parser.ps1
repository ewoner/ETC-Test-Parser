...
$dir = "H:\Documents\Projects\testParser\HTML test files"           #Directory to find html files; defaults to currect directory   TODO: move to CONF file
...
$ObjectivesNumber = 13     #Total number of Objectives, used to create array  TODO: move to CONF file
                    #"Objectives?:\s+$Mod\.(\d+)"
$ObjectivePattern = "Objectives?:\s+$Mod\.(\d+)"   #Regex 'string' to find the objective missed
...
$ObjectiveDisplayStr = "Objective: "  #output string for Console output

#Get files to read
$files = get-childItem -Path $dir -Filter "*.htm*"
...
foreach ( $file in $files ){
    ...
    Write-Debug "Getting file $dir/$file :"
    ...
    $outstr = "";  #output string to a file.
    $Disclaimer = "
    ...
    write-host $outstr
    try {
        Write-host "Creating file '$dir/$namestr.txt' and saving ..... " -nonewline
        new-item -Path $dir -Name "$nameStr.txt" -Force 1> $null
        Set-Content -Path "$dir/$namestr.txt" -Value $outstr
        write-debug "File saved correctly."
        write-host "Succesful!"
    }
    catch {
        write-Host "Error!"
        Write-Error "Unknown error while trying to save file: $dir/$namestr.txt"
    }
