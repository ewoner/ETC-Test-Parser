...
function Import-ModConfiguration {
    ...
    [string] $ConfigDirectory = (Join-Path -Path (Split-Path -Parent $PSScriptRoot) -ChildPath 'Config')  
)
    ...
    if (Test-Path $filePath) {
        ...
        if ($filePath -eq $modConfigFile) {
            ...
            if ($objectivesStart -ge "<objectives>".Length -and $objectivesEnd -ge 0) {
				...
				# Add Objective 0, placeholder of a bad parsed Objective Number.
				$objective = [Objective]::new($modNum, 0, 0, "Objective Number Not Parsed Correctly.")
                ...
                if ($isDailyBlock) {
                    ...
                    foreach ($line in $objectivesArray) {
                        ...
                        if ($trimmedLine -eq "<DAYBREAK>") {
                            ...
                            if ($objNum -gt 0) {
                                $dayNum++  # Increment dayNum only if objectives were processed
                            }
                            $objNum = 0  # Reset objective number for the next day
