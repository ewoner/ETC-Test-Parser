...
param (
    ...
    [string]$debugParser = "on",  # Default value is 'off'
...
function Load-Modules {
    ...
    foreach ($module in $modules) {
        ...
        try {
            if ( Test-path $destinationPathName ) {
				Remove-Item $destinationPathName -Recurse
			}
            ...
            if (($module.Name -eq "EtcWebService" -or $module.Name -eq "TestParser") ) {
			 ...
			 # Import the module
            Write-output "Importing module '$($module.Name)'."
            Import-Module $module.Name -Force -ErrorAction Stop
            Write-Debug "Successfully imported module '$($module.Name)'."
        }
        catch {
...
if ($devParser -eq "home") {
   ...
   #Load-Modules $ImportExcelPath $saveModuleParentPath -verbose
}
...
while (-not $group) {
	...
	elseif ($classNumber -eq 99999 -and $modNumber -eq 13) {
        $classNumber = "24310"
    }
...
foreach ($gradeItem in $course.gradeitems) {
    ...
    if ($gradeItem.graderaw -lt 74.50 -and $gradeItem.graderaw -ne 0.00 ) {
